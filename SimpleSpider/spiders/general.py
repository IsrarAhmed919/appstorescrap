...
class GeneralSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        apps_links=response.xpath('//*[@id="SearchResultsListings"]/div/div/a/@href').getall()
        for i in apps_links:
            link=response.urljoin(i)
            yield scrapy.Request(link, callback=self.app_data)
            self.request_made=True
        
        if self.request_made:
            nxt_pg_url=response.xpath('//*[@id="Main"]/section[2]/div/div[2]/div[4]/*[@class="search-pagination__next-page-text"]/@href').get()
            nxt_pg=response.urljoin(nxt_pg_url)
            print(nxt_pg)
            yield scrapy.Request(nxt_pg, callback=self.parse)
     
      
    data_dict = {}
    def app_data(self, response, data_dict=data_dict):
        #initializing a list
        ...
        #fetch(get_link)
        #response.request(get_link)
        #reviews=get_all_reviews(self, response, get_link)
        ...
        #     # j_data= f.read()
        #     # print(j_data)
        #     #j_data.append(data_list)
        #     #json.dumps(j_data, f, indent=2)
        #     f.write(json.dumps(self.data_list, indent=2))
        #     f.close()
    ...
    def get_reviews(self, response, data_dict=data_dict):
        ...
        if next_pg:
            ...
            f = open('AllAppsData.json', 'r+')
            #f.write(json.dumps(all_jobs)) # all in one line
            j_data=json.load(f)
            j_data=j_data.append(data_dict)
            f.write(json.dumps(j_data, indent=2))
            f.close()
            data_dict={}
